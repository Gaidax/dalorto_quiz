...
(function (core) {
    ...
    function randomQ(key) {
        var newkey = Math.random() * 24 + 1;
        if ((core.counter.length + 1) != 11) {
            if (key != newkey) {
                var new_question = config.Scene_questions[Math.floor(Math.random() * 24) + 1];
                console.log(new_question);
                console.log(core.counter.indexOf(new_question));
                if (core.counter.indexOf(new_question) == -1) {
                    core.counter.push(new_question);
                    core.scene = new_question;
                    changeScene();
                }
                else {
                    randomQ(key);
